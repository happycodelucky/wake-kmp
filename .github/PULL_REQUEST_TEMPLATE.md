...
- [ ] Public API changes follow the Swift-interop rules (§8): `kotlin.Result` + sealed exception hidden from ObjC and bridged to a Swift error enum, `@Throws` lists the domain exceptions (+ `CancellationException` on suspend) with no default args, no `kotlin.Result<T>` in a Swift-visible signature; a Swift-facing change is checked in the built `.swiftinterface`
